    ...
    unsafe fn get_class_info2(&self, index: i32, info: *mut PClassInfo2) -> tresult {
        ...
        match index {
            0 => {
                ...
                strcpy("Cuisine", info.name.as_mut_ptr());
                info.class_flags = 1;
            ...
            1 => {
                ...
                strcpy("Cuisine Controller", info.name.as_mut_ptr());
                info.class_flags = 0;
...
impl IPluginFactory for Factory {
    unsafe fn get_factory_info(&self, info: *mut PFactoryInfo) -> tresult {
        ...
        let info = &mut *info;
        strcpy("Kavelin", info.vendor.as_mut_ptr());
        strcpy("https://kavelin.neocities.org", info.url.as_mut_ptr());
        strcpy("mailto://dev.velp@gmail.com", info.email.as_mut_ptr());
        info.flags = 8;
